...
import com.database.AbBasicDBDao;
import com.database.AbStrUtil;
import com.database.orm.AbTableHelper;
import com.database.orm.annotation.ActionType;
import com.database.orm.annotation.Column;
import com.database.orm.annotation.Id;
import com.database.orm.annotation.Relations;
import com.database.orm.annotation.RelationsType;
import com.database.orm.annotation.Table;
...
public class AbDBDaoImpl<T> extends AbBasicDBDao implements AbDBDao<T> {
	 ...
	 */
	@Override
	synchronized public long insert(T entity) {
	 ...
	 */
	@Override
	synchronized public long insert(T entity, boolean flag) {
		String sql = null;
		long row = 0L;
		try {
			lock.lock();
	 ...
	 */
	@Override
	synchronized public long insertList(List<T> entityList) {
	 ...
	 */
	@Override
	synchronized public long insertList(List<T> entityList, boolean isAutoCreateId) {
		// String sql = null;
		return inserList(entityList, isAutoCreateId, false);
	}
	 ...
	 * @param rows
	 */
	private long inserList(List<T> entityList, boolean flag, boolean isReplace) {
		String sql = null;
		long rows = 0;
		try {
			lock.lock();
			for (T entity : entityList) {
				...
				Log.d(TAG, "[insertList]: insert OR REPLACE into " + this.tableName + " " + sql);
				db = dbHelper.getWritableDatabase();
				if (isReplace) {
					db.execSQL("insert OR REPLACE into " + this.tableName + " " + sql);
					rows++;
				} else {
				...
				if (RelationsType.one2one.equals(type)) {
					...
					if (relationsEntity != null) {
						...
						Log.d(TAG, "[insertList]: insert OR REPLACE1 into " + relationsTableName + " " + sql);
						if (isReplace) {
							db.execSQL("insert OR REPLACE into " + relationsTableName + " " + sql);
							rows++;
						} else {
					...
					if (list != null && list.size() > 0) {
						for (T relationsEntity : list) {
							...
							Log.d(TAG, "[insertList]: insert OR REPLACE2 into " + relationsTableName + " " + sql);
							if (isReplace) {
								db.execSQL("insert OR REPLACE into " + relationsTableName + " " + sql);
								rows++;
							} else {
	...
	@Override
	public long update(String whereClause, String[] whereArgs) {
		long rows = -1;
		try {
			lock.lock();
			String mLogSql = getLogSql(whereClause, whereArgs);
			...
			Log.d(TAG, "[update]: update from " + this.tableName + mLogSql);
			rows = db.update(this.tableName, null, whereClause, whereArgs);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			lock.unlock();
	...
	@Override
	public long insertList(List<T> entityList, boolean isAutoCreateID, boolean isReplace) {
		return inserList(entityList, isAutoCreateID, isReplace);
	}
}
